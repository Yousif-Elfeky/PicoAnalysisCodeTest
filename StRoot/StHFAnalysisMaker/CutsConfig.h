...
namespace HFCuts {
    // -------- Event cuts (common) --------
    namespace Event {
        constexpr float VzMax      = 30.f;   // |Vz| (cm) for collider; ignore for FXT with special window below
        constexpr float VzDiffMax  = 3.f;    // |Vz - VzVPD|
    ...
    // -------- Track quality --------
    namespace Track {
        constexpr int   nHitsFitMin    = 20;   // FXT: 15
        constexpr float nHitsFracMin   = 0.52;
        constexpr float dcaMax         = 1.0;  // FXT: 3.0
        constexpr float ptMin          = 0.2f; // FXT: 0.1
    }
    ...
    namespace PID {
        ...
        constexpr float oneOverBetaMax = 0.03; // 19 GeV, loosen to 0.04 at FXT
